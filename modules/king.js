/* King module */

function renderRoundPills() {
  const c = document.getElementById('round-pills'); c.innerHTML = '';
  for (let r = 1; r <= NUM_ROUNDS; r++) {
    const btn = document.createElement('button');
    btn.className = 'round-pill' + (r===state.currentRound?' active':state.rounds[r]?' done':'');
    btn.textContent = 'Ronda ' + r;
    btn.addEventListener('click', () => { state.currentRound = r; renderKing(); });
    c.appendChild(btn);
  }
}

function renderTotalsBar() {
  const c = document.getElementById('totals-bar'); c.innerHTML = '';
  const rankings = getRankings();
  const medals = ['♛','♜','♝'];
  rankings.forEach((p,i) => {
    const div = document.createElement('div');
    div.className = 'total-card' + (i===0?' rank-1':'');
    const rankLabel = (i<3?medals[i]+' ':'') + (i+1) + 'º lugar';
    div.innerHTML = `
      <div class="avatar" style="background:${p.color}22;color:${p.color}">${getInitials(p.name)}</div>
      <div class="tc-info">
        <input class="player-name-edit" type="text" value="${p.name}" maxlength="14" data-pid="${p.id}" />
        <div class="tc-rank">${rankLabel}</div>
      </div>
      <div class="tc-score">${p.total}</div>
    `;
    const inp = div.querySelector('.player-name-edit');
    inp.addEventListener('blur', () => {
      const n = inp.value.trim();
      if (n && n !== p.name) { renamePlayer(p.id, n); renderKing(); showToast('Nome atualizado!'); }
    });
    inp.addEventListener('keydown', e => { if (e.key==='Enter') inp.blur(); });
    c.appendChild(div);
  });
}

function renderScorecard() {
  const tbl = document.getElementById('scorecard');
  initRound(state.currentRound);
  const rd = state.rounds[state.currentRound];
  const t1 = calcTotal1(state.currentRound);
  const t2 = calcTotal2(state.currentRound);
  const gt = calcGrandTotal(state.currentRound);
  const numP = state.players.length;

  let html = '<thead><tr><th>Categoria</th>';
  state.players.forEach(p => {
    html += `<th><span class="player-color-dot" style="background:${p.color}"></span>${p.name}</th>`;
  });
  html += '</tr></thead><tbody>';

  // Penalties section
  html += `<tr class="row-section-header"><td colspan="${1+numP}">Penalizações (pontos negativos)</td></tr>`;
  PENALTY_ROWS.forEach(pr => {
    const v_per_player = state.players.map(p => rd.penalties[pr.key] ? (rd.penalties[pr.key][p.id]||0) : 0);
    html += `<tr class="row-penalty"><td>${pr.label} <span style="font-size:10px;color:var(--text-muted)">(max ${pr.pts})</span></td>`;
    state.players.forEach((p,i) => {
      html += `<td><input class="score-cell" type="number" min="0" max="${pr.pts}" value="${v_per_player[i]}" data-type="pen" data-key="${pr.key}" data-pid="${p.id}" /></td>`;
    });
    html += '</tr>';
  });

  html += `<tr class="row-total"><td>Total 1 (pontos negativos)</td>`;
  state.players.forEach(p => {
    html += `<td class="computed ${scoreClass(t1[p.id],false)}" data-total1="${p.id}">${fmt(t1[p.id])}</td>`;
  });
  html += '</tr>';

  // Festas section
  html += `<tr class="row-section-header"><td colspan="${1+numP}">Festas (pontos positivos)</td></tr>`;
  rd.festas.forEach((f,i) => {
    html += `<tr class="row-festa"><td><input class="row-label-edit" type="text" value="${f.label}" data-fi="${i}" /></td>`;
    state.players.forEach(p => {
      html += `<td><input class="score-cell" type="number" value="${f.values[p.id]||0}" data-type="festa" data-fi="${i}" data-pid="${p.id}" /></td>`;
    });
    html += '</tr>';
  });

  html += `<tr class="row-total2"><td>Total 2 (pontos positivos)</td>`;
  state.players.forEach(p => {
    html += `<td class="computed ${scoreClass(t2[p.id],true)}" data-total2="${p.id}">${fmt(t2[p.id])}</td>`;
  });
  html += '</tr>';

  html += `<tr class="row-grandtotal"><td>♛ Soma Total</td>`;
  state.players.forEach(p => {
    html += `<td class="computed c-gold" data-grandtotal="${p.id}">${fmt(gt[p.id])}</td>`;
  });
  html += '</tr></tbody>';

  tbl.innerHTML = html;

  tbl.querySelectorAll('.score-cell').forEach(el => el.addEventListener('input', onCellInput));
  tbl.querySelectorAll('.row-label-edit').forEach(el => {
    el.addEventListener('blur', () => {
      const fi = parseInt(el.dataset.fi);
      const label = el.value.trim();
      if (label) setFestaLabel(state.currentRound, fi, label);
    });
    el.addEventListener('keydown', e => { if (e.key==='Enter') el.blur(); });
  });

  document.getElementById('btn-remove-festa').style.display = rd.festas.length > 1 ? '' : 'none';
}

function onCellInput(e) {
  const el = e.target;
  const pid = parseInt(el.dataset.pid);
  const v = parseInt(el.value) || 0;
  const r = state.currentRound;
  initRound(r);
  if (el.dataset.type === 'pen') {
    const max = PENALTY_ROWS.find(pr => pr.key===el.dataset.key).pts;
    const clamped = Math.min(Math.max(0,v), max);
    state.rounds[r].penalties[el.dataset.key][pid] = clamped;
    el.value = clamped;
  } else {
    state.rounds[r].festas[parseInt(el.dataset.fi)].values[pid] = v;
  }
  saveState();
  updateComputedTotals();
}

function updateComputedTotals() {
  const r = state.currentRound;
  const t1=calcTotal1(r), t2=calcTotal2(r), gt=calcGrandTotal(r);
  state.players.forEach(p => {
    const c1=document.querySelector(`[data-total1="${p.id}"]`);
    const c2=document.querySelector(`[data-total2="${p.id}"]`);
    const cg=document.querySelector(`[data-grandtotal="${p.id}"]`);
    if(c1){c1.textContent=fmt(t1[p.id]);c1.className=`computed ${scoreClass(t1[p.id],false)}`;}
    if(c2){c2.textContent=fmt(t2[p.id]);c2.className=`computed ${scoreClass(t2[p.id],true)}`;}
    if(cg){cg.textContent=fmt(gt[p.id]);cg.className='computed c-gold';}
  });
  renderTotalsBar();
}

function renderKing() {
  renderRoundPills();
  renderTotalsBar();
  renderScorecard();
}

// Add player modal
let selectedColor = PALETTE[0];

function openAddPlayerModal() {
  selectedColor = PALETTE[state.players.length % PALETTE.length];
  renderColorPicker();
  document.getElementById('pname-inp').value = '';
  openModal();
  setTimeout(() => document.getElementById('pname-inp').focus(), 80);
}

function renderColorPicker() {
  const c = document.getElementById('cpicker'); c.innerHTML = '';
  PALETTE.forEach(color => {
    const d = document.createElement('div');
    d.className = 'cdot' + (color===selectedColor?' sel':'');
    d.style.background = color;
    d.addEventListener('click', () => { selectedColor=color; renderColorPicker(); });
    c.appendChild(d);
  });
}

function confirmAddPlayer() {
  const name = document.getElementById('pname-inp').value.trim();
  if (!name) { showToast('Insere um nome'); return; }
  if (state.players.find(p => p.name.toLowerCase()===name.toLowerCase())) { showToast('Nome já existe'); return; }
  addPlayer(name, selectedColor);
  closeModal();
  renderKing();
  showToast(name + ' adicionado!');
}
